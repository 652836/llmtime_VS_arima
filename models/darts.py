...
def _resolve_darts_devices(accelerator):
    raw_value = _env_text('LLMTIME_DARTS_DEVICES')
...
def get_darts_pl_trainer_kwargs():
    accelerator = _env_text('LLMTIME_DARTS_ACCELERATOR')
    if accelerator is None:
        accelerator = 'gpu' if torch.cuda.is_available() else 'cpu'
    accelerator = accelerator.lower()
    devices = _resolve_darts_devices(accelerator)
    max_steps = _env_int('LLMTIME_DARTS_MAX_STEPS', 10000)
    trainer_kwargs = {
...
def get_chunked_AR_predictions_data(modeltype,train,test, epochs=400, in_len=12, out_len=12, likelihood='laplace', num_samples=100, n_train=None, **kwargs):
    ...
    model = modeltype(
        ...
        pl_trainer_kwargs=get_darts_pl_trainer_kwargs(),
        **kwargs
    )
...
def get_chunked_AR_likelihoods(model,scaled_series,trainsize,in_len,out_len,scaler):
    ...
    nll_per_dimension -= np.log(scaler.scale_)
